...
def detect_misplaced_class(g, class_list):
    ...
    for class_uri in class_list:
        ...
        temp_graph = Graph()
        temp_graph += g.triples((None, class_uri, None))
...
def detect_misplaced_property(g, property_list):
    ...
    # Filter property because these properties are both owl:Class and rdfs:Property
    test_property_list = [item for item in property_list if item not in PropertyList]

    for property_uri in test_property_list:
        # Get subset of RDF graph using this property as the object of rdf:type triple
        temp_graph = Graph()
        temp_graph += g.triples((None, RDF.type, property_uri))
